...
test("a brief posts as the bot even when only a user token is set", () => {
  ...
  // The only calls that ask for the write credential are the four that write: posting a message, editing
  // one (the Slack assistant rewrites its reply in place as the answer forms), and adding and removing the
  // :eyes: reaction that marks the bot working. Reads must never reach for the write token, so this count
  // guards against a read quietly acquiring it.
  assert.equal(slackLib.match(/\}, "write"\);/g)?.length, 4);
