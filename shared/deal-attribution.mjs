 ...
 * can say exactly why.
 *
 * @param {{ leads?: Array<{linkedin?: string, email?: string, campaign?: string, name?: string}>, meetings?: Array<{email?: string, linkedin?: string, campaign?: string, domain?: string, name?: string}> }} sources
 */
export function buildQcIdentity(sources) {
  ...
  for (const lead of sources?.leads ?? []) {
    add(byEmail, normalizeEmail, lead.email, { source: "campaign", campaign: lead.campaign || "", name: lead.name || "" });
    add(byLinkedin, normalizeLinkedin, lead.linkedin, { source: "campaign", campaign: lead.campaign || "", name: lead.name || "" });
  }
  ...
  for (const meeting of sources?.meetings ?? []) {
    ...
    if (email) byEmail.set(email, { source: "meeting", campaign: meeting.campaign || "", name: meeting.name || "" });
    const linked = normalizeLinkedin(meeting.linkedin);
    if (linked) byLinkedin.set(linked, { source: "meeting", campaign: meeting.campaign || "", name: meeting.name || "" });
    const domain = normalizeDomain(meeting.domain);
    if (domain) domains.add(domain);
  }
  return { byEmail, byLinkedin, domains };
}
...
export function attributeDeal(deal, qc) {
  ...
  // Weaker: the same company, but nobody on the deal ties back. Flagged, not counted.
  ...
  if (domain && domains.has(domain)) {
    return {
      ...
      matchedBy: "domain",
      campaign: "",
      reason: `${deal?.companyName || domain} is a company QC has campaigned into, but no specific person on this deal matched — worth a look.`,
      evidence: { domain },
    };
  }
