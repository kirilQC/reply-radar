...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, workspace: { id: row.id, name: row.name, slug: row.slug, brief: row.client_brief ?? "", model: row.anthropic_model ?? "", systemPrompt: row.custom_system_prompt ?? "", messagingDocUrl: guardrails.messaging_doc_url ?? "", quickTemplates: cleanTemplates(guardrails.quick_templates) } });
  } catch (error) {
