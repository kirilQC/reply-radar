...
export async function POST(request: Request, context: { params: Promise<{ workspaceId: string }> }) {
  ...
  try {
    ...
    const lookup = await fetch(`${url}/rest/v1/rr_workspaces?select=id,slug,heyreach_api_key_ciphertext,guardrails&${lookupColumn}=eq.${encodeURIComponent(workspaceId)}&limit=1`, { headers, cache: "no-store" });
    if (!lookup.ok) return NextResponse.json({ ok: false, stage: "workspace_lookup", error: (await lookup.text()).slice(0, 1_000) }, { status: 502 });
    const rows = await lookup.json() as Array<{ id: string; heyreach_api_key_ciphertext?: string | null; guardrails?: Record<string, unknown> | null }>;
