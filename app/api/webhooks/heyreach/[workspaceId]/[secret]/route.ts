...
export async function POST(request: Request, context: { params: Promise<{ workspaceId: string; secret: string }> }) {
  ...
  try {
    ...
    const lookup = await fetch(`${url}/rest/v1/rr_workspaces?select=id,slug,webhook_secret_hash,heyreach_api_key_ciphertext&${lookupColumn}=eq.${encodeURIComponent(workspaceId)}&limit=1`, { headers, cache: "no-store" });
    if (!lookup.ok) return NextResponse.json({ ok: false, stage: "workspace_lookup", error: (await lookup.text()).slice(0, 1_000) }, { status: 502 });
    const rows = await lookup.json() as Array<{ id: string; webhook_secret_hash?: string | null; heyreach_api_key_ciphertext?: string | null }>;
