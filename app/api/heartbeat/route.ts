...
export async function GET() {
  ...
  const aiArkEnabled = ["1", "true", "yes", "on"].includes(String(process.env.AI_ARK_ENRICHMENT_ENABLED ?? "").trim().toLowerCase());
  const services = [
    ...
    { id: "ai_ark", label: "AI Ark enrichment", configured: !aiArkEnabled || Boolean(process.env.AI_ARK_API_KEY), explanation: aiArkEnabled ? "Adds verified profile and company details to new LinkedIn leads." : "Turned off globally by AI_ARK_ENRICHMENT_ENABLED." },
  ];
  ...
  try {
    ...
    const [workspaceResult, syncResult, eventResult, schemaResult, aiArkResult, recentLeadResult] = await Promise.all([
    ...
    const aiArkRuns = Array.isArray(aiArkResult.body) ? aiArkResult.body as Row[] : [];
    const recentLeads = Array.isArray(recentLeadResult.body) ? recentLeadResult.body as Row[] : [];
    ...
    const aiArkFailures24h = aiArkRuns.filter((run) => run.status === "failed" && new Date(String(run.started_at)).getTime() >= new Date(since).getTime());
    const aiArkSuccesses24h = aiArkRuns.filter((run) => run.status === "success" && new Date(String(run.started_at)).getTime() >= new Date(since).getTime());
    const unenrichedLeads24h = recentLeads.filter((lead) => {
      const raw = lead.raw_data && typeof lead.raw_data === "object" && !Array.isArray(lead.raw_data) ? lead.raw_data as Row : {};
      const metadata = raw.reply_radar && typeof raw.reply_radar === "object" && !Array.isArray(raw.reply_radar) ? raw.reply_radar as Row : {};
      const enrichment = metadata.ai_ark && typeof metadata.ai_ark === "object" && !Array.isArray(metadata.ai_ark) ? metadata.ai_ark as Row : {};
      return Object.keys(enrichment).length === 0;
    });
    const aiArkProblemCount = Math.max(aiArkFailures24h.length, unenrichedLeads24h.length);
    const aiArkStatus = !aiArkEnabled ? "disabled" : !process.env.AI_ARK_API_KEY ? "not_configured" : !aiArkResult.response.ok || !recentLeadResult.response.ok || aiArkProblemCount > 5 ? "attention" : "healthy";
    ...
    const aiArk = {
      ...
      explanation: aiArkStatus === "disabled" ? "Enrichment is intentionally turned off in Vercel." : aiArkStatus === "not_configured" ? "Enrichment is on, but the AI_ARK_API_KEY is missing." : !aiArkResult.response.ok || !recentLeadResult.response.ok ? "The health check could not read AI Ark run or lead data from Supabase." : aiArkStatus === "attention" ? `More than 5 recent leads could not be enriched. Check the failures below.` : "AI Ark is enriching leads normally.",
      recentFailures: aiArkFailures24h.slice(0, 10).map((run) => ({ startedAt: run.started_at, workspaceId: run.workspace_id, error: run.error_text })),
      recentRuns: aiArkRuns.slice(0, 25),
    };
    return NextResponse.json({ status: "live", services, clients, worker, aiArk, checkedAt, thresholds, diagnostics: {
      runtime: { node: process.version, supabaseUrlConfigured: true, serviceRoleKeyConfigured: true, anthropicKeyConfigured: Boolean(process.env.ANTHROPIC_API_KEY), aiArkKeyConfigured: Boolean(process.env.AI_ARK_API_KEY), aiArkEnrichmentEnabled: aiArkEnabled, workerServiceUrlConfigured: Boolean(process.env.WORKER_SERVICE_URL), pollIntervalSeconds: Number(process.env.POLL_INTERVAL_SECONDS || 120) },
      queries: {
        ...
        aiArkRuns: { status: aiArkResult.response.status, ok: aiArkResult.response.ok, durationMs: aiArkResult.durationMs, rowCount: aiArkRuns.length, error: aiArkResult.response.ok ? null : aiArkResult.body },
        recentLeads: { status: recentLeadResult.response.status, ok: recentLeadResult.response.ok, durationMs: recentLeadResult.durationMs, rowCount: recentLeads.length, error: recentLeadResult.response.ok ? null : recentLeadResult.body },
