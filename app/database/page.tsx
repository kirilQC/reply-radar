  ...
  return (
    <div className="app-shell">
      ...
      {selectedId && <div className="database-drawer-backdrop"><aside className="database-drawer" aria-label="Lead details">
        <div className="database-drawer-head"><div>{selectedSummary?.photoUrl ? <i><img src={selectedSummary.photoUrl} alt="" /></i> : <i>{initials(selectedSummary?.name || "")}</i>}<span><h2>{selectedSummary?.name || "Loading…"}</h2></span></div><button onClick={() => setSelectedId(null)} aria-label="Close lead details">×</button></div>
...
function ExperienceTimeline({ groups, currentCompany }: { groups: unknown[]; currentCompany: Record<string, unknown> }) {
  ...
  return <section><h3>Experience</h3><div className="database-experience-list">{groups.map((item, index) => {
    ...
    const roles = asList(group.profile_positions || group.positions || group.position).map(asObject); const isCurrentCompany = Boolean(currentName && companyName.toLowerCase() === currentName);
    ...
    return <article key={`${companyName}-${index}`}><header>{Boolean(company.logo) && <img src={String(company.logo)} alt="" />}<div><strong>{companyName}</strong>{industry && <span>{industry}</span>}</div>{destination && <a className="database-company-link" href={destination} target="_blank" rel="noreferrer">View company ↗</a>}</header>{roles.length ? roles.map((role, roleIndex) => { const dates = asObject(role.date); const start = dates.start || groupDate.start; const end = dates.end || groupDate.end; return <div className="database-role" key={`${text(role.title)}-${roleIndex}`}><h4>{display(role.title || role.name)}</h4><p className="database-role-dates">{monthYear(start)} – {monthYear(end)}{start ? ` · ${durationBetween(start, end)}` : ""}</p></div>; }) : <div className="database-role"><p className="database-role-dates">{monthYear(groupDate.start)} – {monthYear(groupDate.end)}{groupDate.start ? ` · ${durationBetween(groupDate.start, groupDate.end)}` : ""}</p></div>}</article>;
  })}</div></section>;
}
