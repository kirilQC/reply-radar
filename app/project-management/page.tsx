      ...
      <section className="main-area">
        ...
        <main className="pm-shell pm-directory-shell">
          ...
          <div className="pm-directory">
            {clients.map((c) => (
              <div className="pm-card pm-editcard" key={c.id}>
...
function ViewEditor({ view, clients, onClose, onSaved }: { view: ViewDef | null; clients: Client[]; onClose: () => void; onSaved: () => void }) {
  ...
  const [channel, setChannel] = useState(view?.slackChannelId ?? "");
  ...
  const save = async () => {
    ...
    setBusy(true); setErr("");
    const body = { name: name.trim(), memberSlugs: members, logoUrl: logoUrl.trim() || null, slackChannelId: channel.trim(), ...(view ? { id: view.id } : {}) };
