...
export type Project = { id: string; title: string; stage: string; assignee: string | null; priority: string | null; week: string | null; dueDate: string | null; context: string | null; links: ProjectLink[]; source: string; createdAt: string; updatedAt: string };
...
const shape = (r: Row): Project => ({ id: String(r.id), title: String(r.title ?? ""), stage: String(r.stage ?? "todo"), assignee: (r.owner as string) || null, priority: (r.priority as string) || null, week: (r.week as string) || null, dueDate: (r.due_date as string) || null, context: (r.context as string) || null, links: Array.isArray(r.links) ? (r.links as ProjectLink[]) : [], source: String(r.source ?? "manual"), createdAt: String(r.created_at ?? ""), updatedAt: String(r.updated_at ?? "") });
