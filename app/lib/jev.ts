...
- A generic title ("Director", "VP", "Associate Director of Operations") is never grounds for "unclear" on its own. Any question about what the person does NOW must tell Jev to look at every field before answering: the current role's title and \`about\`, \`headline\`, \`about\`, \`skills\`, \`department\`, \`other\` (which can hold publication titles), and the most recent \`past_roles\` — a recent role doing the target work at the SAME employer, with nothing suggesting a change of function, is evidence the current role continues it. "Unclear" is only for when none of those say anything either way.
...
- Refer to profile fields by name in backticks. Every contact list, whatever tool exported it, is mapped onto this one profile shape, and a question set is reused across lists, so name only these fields:
  ...
  \`other\` — extra columns by their original header (e.g. publication titles)
