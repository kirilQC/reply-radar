...
export type ProjectView = { id: string; name: string; slug: string; logoUrl: string | null; accentColor: string | null; memberSlugs: string[] };
...
const shape = (v: Row): ProjectView => ({ id: String(v.id ?? ""), name: String(v.name ?? ""), slug: String(v.slug ?? ""), logoUrl: (v.logoUrl as string) || null, accentColor: (v.accentColor as string) || null, memberSlugs: Array.isArray(v.memberSlugs) ? (v.memberSlugs as unknown[]).map(String) : [] });
...
export async function createView(input: { name: string; memberSlugs: string[]; logoUrl?: string | null; accentColor?: string | null }): Promise<{ ok: boolean; error?: string; view?: ProjectView }> {
  ...
  const view: ProjectView = { id: `pv_${Date.now().toString(36)}`, name: name.slice(0, 80), slug, logoUrl: input.logoUrl || null, accentColor: input.accentColor || null, memberSlugs: members };
  if (!(await save([...views, view], c))) return { ok: false, error: "Could not save the view." };
  return { ok: true, view };
}
export async function updateView(id: string, fields: Partial<{ name: string; memberSlugs: string[]; logoUrl: string | null; accentColor: string | null }>): Promise<{ ok: boolean; error?: string }> {
  const c = creds(); if (!c) return { ok: false, error: "Supabase not configured" };
  const views = await listViews();
  const idx = views.findIndex((v) => v.id === id); if (idx < 0) return { ok: false, error: "View not found." };
  const cur = views[idx];
  views[idx] = { ...cur, name: fields.name !== undefined ? String(fields.name).slice(0, 80) : cur.name, memberSlugs: fields.memberSlugs !== undefined ? Array.from(new Set(fields.memberSlugs.map(String).filter(Boolean))) : cur.memberSlugs, logoUrl: fields.logoUrl !== undefined ? fields.logoUrl : cur.logoUrl, accentColor: fields.accentColor !== undefined ? fields.accentColor : cur.accentColor };
