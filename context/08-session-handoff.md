...
- **Granola box on the automations page is now real.** It was ticked whenever keys existed and a title was set. The page now calls `/api/granola/coverage` (one list pass across every key, `callCoverage` in `granola.ts`) and ticks only when a matching call was found in the 14-day window, showing its date and which keys saw it; "No call in 14 days" otherwise. The worker never calls this route and `readiness.ready` / `dueNow` are unchanged, so a client with no call still gets its brief.
- **Shared notes are credited to their owner.** Ben's notes are workspace-visible, so every key with the "Public notes" scope returns them. The key Test now lists only the holder's own notes (matched on owner vs label) and counts the rest as "shared by …"; the automations Granola box names who *recorded* a matching call rather than whose key returned it. Briefs still read shared notes, so a call is found even if its recorder's key is missing.
- **Brief divider is 32 `=`** (was 37); headings re-centre from `BRIEF_DIVIDER.length`, and the Friday EOW reminder now sits flush left because it is wider than the rule.
- **Morning brief needs both Slack channels by default.** `readinessOf` fails a missing external channel ("No external channel") unless `guardrails.slack_internal_only` is true — set by the "This client only has an internal channel" checkbox under the Slack channel ids in client settings. Stored in guardrails, so no migration. EOW and call analysis are unchanged (internal alone still passes).
