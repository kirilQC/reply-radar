...
- **Cleaned list, original columns** (primary contact download): the upload's own headers, order and cell values, nothing added, minus bad fits. Column roles never affect exports — a LinkedIn column set to "not sent" still comes back. Duplicates follow their first copy's verdict (`duplicateOf`). No BOM on this file, so the first header re-imports cleanly. The other downloads still carry every Jev column.
- **Must-haves are gates, not votes (weighted mode).** A director check at 99% was averaged in and pushed ~90% of a curated Vitalic list to good fit. Now a must only drops (clear fail) or holds back as Maybe (unsure); the score is signals alone.
- **Good fit = most signals answered** (more than half not can't-tell), average ≥ keep, every must ≥ keep. Replaces "any can't-tell → Maybe", which held back "Director, Medicare Stars" for having no past roles.
- **Curated lists:** the question writer no longer treats the employer as evidence when the brief says companies are pre-chosen (it had been told "an employer that serves seniors implies Medicare exposure"). Always-keep terms skip company columns — on a Medicare company list every "Company Product and Services" cell mentions Medicare Advantage.
- `evaluateOne` retries once when Jev's reply leaves out a question (seen live, ~1 in 10 on a 4-question set), keeping the partial reply if the retry fails.
- **Key questions** (`kind: "key"`): required for a good fit, never drop anyone alone (otherwise a vote). A confirmed key also satisfies the "most signals answered" rule. For Vitalic, "current role works on MA" is key — past roles and headline were carrying contacts whose current role Jev couldn't place.
- **Good-fit reasons**: a good verdict's reason lists the questions answered in the contact's favour, strongest first (`fitReason`). **Check good fits with Claude** reuses the contact review with `stage: "good"` — an audit for false positives (drop when the fit rests on employer, seniority or a guess); its one-sentence reason shows in the Why column and the export's "Claude review".
- `normalizeQuestionSet` never lets a "no"-style option be neutral, even if the writer listed it — a build marked `no` neutral and a clear no became a maybe. Repairs saved sets on load.
- **Generic titles:** the question writer must tell Jev, for any "what do they do now" question, to read every field (current role about, headline, about, skills, department, `other`, recent past roles — a recent target-work role at the same employer counts) and answer unclear only when none speak. Tested with eight contacts all titled "Director", evidence hidden in one column each: all five nuggets (headline, skills, current description, same-employer past role, publication) found; empty → Maybe; IT → Bad.
- **Full AI Ark history:** `MAX_PAST_ROLES` 8 → 16 (28 of 113 sample contacts had more than 8); "Bottom Experience" (oldest job) is read as experience #999 and de-duplicated against the numbered ones; publication titles go to `other`.
- **Claude review instructions** (`reviewBrief` on the question set; box above the contact downloads once a run finishes): the team's post-run rules ("sales roles are out"), passed to the contact review as overriding the brief. Saving it doesn't clear results; a rebuild keeps it. While it is set, Claude's drop beats always-keep protection — a Medicare sales director's headline says "Medicare Advantage", so protection kept exactly the rows the rules remove. Contact review runs 6 batches in parallel (was 3). Live test: Medicare sales, BD and marketing directors all dropped (high confidence); program roles kept with the deciding fact.
- **Cleaned list + Jev reasons**: the cleaned download now appends Jev's decision columns (verdict, fit score, reason, note, always-keep match, Claude review, and per question the chosen option, fit % and confidence) *after* the upload's own columns, which stay first and in order. Shared `decisionHeaders` / `decisionCells` with the "Everything, with verdicts" export. Duplicates carry their first copy's decision.
- **Setup and results can no longer drift apart.** A Bluevia export had the previous questions' verdicts under the new questions' column headings (answer columns blank): keep-terms / review-instructions / scoring saves sent the whole set as the tab held it, so a stale tab could restore old questions. Those saves are now `PUT { patch }`, merged server-side into the stored set. `run()` takes the saved set from its build probe (switching the tab and saying so if it changed), and both contact downloads label answer columns with `runQuestions` — the questions the run actually used.
